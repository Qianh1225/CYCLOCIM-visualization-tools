load sol_4x4_3000_v1_uv_baro.mat
 ...
 % v = g*x-e*y;
 u_temp = output.M3d*0;
 u_temp(output.msk.ukeep) = output.u_geo(:,1);
 v_temp = output.M3d*0;
 v_temp(output.msk.ukeep) = output.v_geo(:,1);
 u = u_temp(:,:,1);
 v = v_temp(:,:,1);
 [phi,psi] = flowfun(u,v);  % Here comes the potential and streamfun.

 contourf(psi,30, 'linestyle','none')    % Contours of streamfunction
 colorbar
 caxis([-0.5,0.5])
 hold on
